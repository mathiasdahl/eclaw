...
(defvar eclaw-system-prompt
  (concat
   ...
   "You are running inside Emacs. Local filesystem tools (`read_file', "
   "`list_directory', `glob_files', `grep_files') are confined to "
   "`eclaw-folder' (paths are absolute or relative to that root). When the user "
   "needs project or code context, use `glob_files' to find files by name, "
   "`grep_files' to search "
   "contents (default `output_mode' files_with_matches; use `content' for line "
   "numbers, then `read_file' with optional `offset'/`limit' for a line range), and "
