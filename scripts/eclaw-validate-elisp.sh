...
run_smoke() {
  ...
  case "$name" in
    ...
    buffer-read)
      ...
      ;;
    emacs-context)
      emacs -batch -Q -L "$REPO_ROOT" -l "$SCRIPT_DIR/smoke/emacs-context.el"
      ;;
    describe-symbol)
      emacs -batch -Q -L "$REPO_ROOT" -l "$SCRIPT_DIR/smoke/describe-symbol.el"
      ;;
    eval-elisp)
    ...
    *)
      ...
      echo "Known: read-file, load, web-search, session-context, archive-utf8, get-datetime, progress-timestamp, send-email, buffer-read, emacs-context, describe-symbol, eval-elisp, web-settings-json" >&2
      exit 2
      ;;
  esac
}
